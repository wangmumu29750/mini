...
- `CLERK` users and `ADMIN` users can sell tickets for registered passengers through the backend service layer. The clerk flow must verify passenger identity against an existing verified passenger profile, create the order under the passenger account, and complete payment/ticket issuing through the existing order service so the passenger can see the issued ticket after login.
