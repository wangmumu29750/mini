...
const navItems = computed(() => {
  if (authStore.role === 'ADMIN') {
    return [
      { to: '/clerk', label: '售票员工作台', icon: '售', show: true },
      { to: '/admin', label: '管理后台', icon: '管', show: true },
      { to: '/admin/settings', label: '系统设置', icon: '设', show: true },
    ]
  }

  if (authStore.role === 'CLERK') {
    return [{ to: '/clerk', label: '售票员工作台', icon: '售', show: true }]
  }

  return [
    { to: '/', label: '车次查询', icon: '查', show: true },
    { to: '/orders', label: '我的订单', icon: '单', show: authStore.isAuthenticated, badge: notificationStore.pendingOrderCount },
    { to: '/tickets', label: '我的车票', icon: '票', show: authStore.isAuthenticated, badge: notificationStore.activeTicketCount },
  ]
})

const roleLabel = computed(() => {
  if (authStore.role === 'ADMIN') return 'ADMIN 管理员账号'
  if (authStore.role === 'CLERK') return 'CLERK 售票员账号'
  return 'PASSENGER 旅客账号'
})
  ...
  <div class="min-h-screen bg-[#f8fafc] text-slate-900">
    <aside class="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-slate-200 bg-white px-7 py-8 shadow-sm lg:block">
      <RouterLink to="/" class="flex items-center gap-4">
        <span class="flex h-14 w-14 items-center justify-center rounded-lg bg-teal-500 text-xl font-bold text-white shadow-lg shadow-teal-200">
          车
        </span>
        <span>
      ...
      <nav class="mt-12 space-y-2">
        ...
        >
          <span class="flex h-8 w-8 items-center justify-center rounded-md bg-slate-100 text-sm leading-none group-hover:bg-teal-100">{{ item.icon }}</span>
          <span>{{ item.label }}</span>
          <span v-if="'badge' in item && item.badge" class="ml-auto rounded-full bg-rose-100 px-2 py-0.5 text-xs text-rose-600">{{ item.badge }}</span>
        </RouterLink>
      </nav>
    </aside>
      ...
      <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
        <div class="flex min-h-20 items-center justify-between gap-4 px-4 sm:px-8">
          <RouterLink to="/" class="flex items-center gap-3 lg:hidden">
            <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-teal-500 font-bold text-white">车</span>
          ...
          <div class="ml-auto flex items-center gap-3">
            <template v-if="authStore.isAuthenticated">
              <div class="hidden text-right sm:block">
                ...
                <div class="mt-1 rounded-md bg-emerald-50 px-2 py-0.5 text-xs font-bold text-emerald-600">{{ roleLabel }}</div>
              </div>
              <div class="relative flex h-12 w-12 items-center justify-center rounded-full border border-slate-200 bg-white text-lg font-black shadow-sm">
